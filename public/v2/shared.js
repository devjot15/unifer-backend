...
(function () {
  ...
  /* Stage 6C-fix-4: capture a live DOM node directly. No off-screen clone — the node is
     serialized in place with its applied styles, which is what html-to-image needs to
     render a non-blank canvas. */
  async function _captureLiveNodeAsCanvas(node) {
    if (!node) throw new Error('captureLiveNode: node is null');
    ...
    const canvas = await window.htmlToImage.toCanvas(node, {
      ...
      filter: (n) => {
        ...
        if (n.classList.contains('low-results-banner')) return false;
        if (n.classList.contains('whatif')) return false;
        if (n.classList.contains('preview-chrome')) return false;
        if (n.id === 'pdf-generating-overlay') return false;
        return true;
      }
    });
  ...
  window.UNIFER.downloadPdf = async function() {
    ...
    try {
      ...
      // ── Capture Page 1: Ranked view ──
      if (originalTab !== 'matches') body.setAttribute('data-tab', 'matches');
      body.setAttribute('data-view', 'ranked');

      // Wait for layout + any in-flight animations
      ...
      // Ensure score bars are filled (they use rAF to set width)
      document.querySelectorAll('.score-fill').forEach(f => {
        if (f.dataset && f.dataset.w) f.style.width = f.dataset.w + '%';
      });
      await new Promise(r => setTimeout(r, 150));

      const rankedCanvas = await _captureLiveNodeAsCanvas(rankedNode);

      // ── Capture Page 2: Compare view ──
      body.setAttribute('data-view', 'compare');
      await new Promise(r => setTimeout(r, 300));

      const compareNode = document.querySelector('.compare-wrap, [data-view-content="compare"], #compareView');
      if (!compareNode) throw new Error('Compare view DOM not found');

      // If Compare has sub-toggles, force all three to be visible simultaneously.
      const compareSubs = compareNode.querySelectorAll('[data-compare-sub], .compare-sub-table, .compare-sub-bars, .compare-sub-strips');
      const hiddenStates = [];
      compareSubs.forEach(sub => {
        hiddenStates.push({ el: sub, display: sub.style.display, visibility: sub.style.visibility });
        sub.style.display = 'block';
        sub.style.visibility = 'visible';
      });
      await new Promise(r => setTimeout(r, 200));

      const compareCanvas = await _captureLiveNodeAsCanvas(compareNode);

      hiddenStates.forEach(({ el, display, visibility }) => {
      ...
      // ── Build PDF with chrome drawn via jsPDF directly ──
      ...
      function drawChrome(pageNum, totalPages, includeSummary) {
        // Watermark — faded "unifer" text, centered, rotated
        pdf.saveGraphicsState();
        pdf.setGState(new pdf.GState({ opacity: 0.05 }));
        pdf.setTextColor(10, 138, 122);
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(140);
        pdf.text('unifer', pdfW / 2, pdfH / 2, {
          align: 'center',
          baseline: 'middle',
          angle: 20
        });
        ...
        // Header: logo square + wordmark + title + subtitle
        ...
        if (includeSummary && filterSummary) {
          pdf.setTextColor(74, 90, 90);
          pdf.setFont('helvetica', 'italic');
          pdf.setFontSize(9);
          const lines = pdf.splitTextToSize(filterSummary, contentW);
        ...
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(8);
        pdf.text(`Page ${pageNum} of ${totalPages}`, margin, pdfH - footerH + 5);
        pdf.text('Generated by UNIFER · unifer.app', pdfW - margin, pdfH - footerH + 5, { align: 'right' });
      }

      function addCanvasWithChrome(canvas, pageNum, totalPages, includeSummary) {
        if (pageNum > 1) pdf.addPage();
        drawChrome(pageNum, totalPages, includeSummary);

        const topOffset = includeSummary ? margin + 30 : margin + 23;
        const availH = pdfH - topOffset - footerH - 4;
      ...
      addCanvasWithChrome(rankedCanvas, 1, 2, true);
      addCanvasWithChrome(compareCanvas, 2, 2, false);
