...
app.post("/recommend", async (req, res) => {
  try {
    ...
    // Duration soft fallback — if fewer than 5 unique universities, re-run without duration filter
    if (primaryTop.length < 5) {
      ...
      if (answers.gre_filter === 'Without GRE or GMAT') {
        softQuery = softQuery.eq('gre_required', false).eq('gmat_required', false);
      } else if (answers.gre_filter === 'Without GRE') {
        softQuery = softQuery.eq('gre_required', false);
      } else if (answers.gre_filter === 'Without GMAT') {
        softQuery = softQuery.eq('gmat_required', false);
      }
      if (answers.profile_gpa_percentage) {
        softQuery = softQuery.or('min_gpa_percentage.is.null,min_gpa_percentage.lte.' + answers.profile_gpa_percentage);
      }
      if (answers.profile_backlogs && parseInt(answers.profile_backlogs) > 0) {
        softQuery = softQuery.neq('accepts_backlogs', false);
      }
      if (answers.selected_country) {
        const { data: scRow } = await supabase.schema('core').from('countries').select('id').eq('name', answers.selected_country).single();
        if (scRow) {
          const { data: scUnis } = await supabase.schema('core').from('universities').select('id').eq('country_id', scRow.id);
          if (scUnis && scUnis.length > 0) {
            softQuery = softQuery.in('university_id', scUnis.map(u => u.id));
          }
        }
      }

      const { data: softCourses } = await softQuery;

      if (softCourses && softCourses.length > 0) {
        // Filter out courses from universities already in primaryTop and courses within the original duration band
        const extraCourses = softCourses.filter(c => {
          const uni = universities.find(u => u.id === c.university_id);
          if (!uni) return false;
          if (excludedUniversities.has(uni.name)) return false;
          // exclude courses already in original hard filter range
          if (c.duration_years >= dBand.min && c.duration_years <= dBand.max) return false;
          return true;
        });
        ...
        const softPathways = await Promise.all(extraCourses.map(async (course) => {
          ...
          const countryScore = computeCountryScore(country, answers);
          ...
          // Duration penalty — 0.10 reduction per year outside preferred band
